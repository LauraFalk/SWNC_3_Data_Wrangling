...
*1. I do not have a Github account* You can still follow along with the training. Rather than branching (making an editable copy of the training that you can send back) you will clone (work on your local machine). We'll go over how to do this. Follow steps in 3 to gain local access to the repo.
...
Please send Laura your github username so that I can add you as a collaborator on this repo. Follow the steps in 3 to gain local access to the repo.
...
nrow(CRMO_ForMerge %>%
        ...
        distinct())
```
Since nrow = 127, we can leave all of the other data in the CRMO dataset. Now, we can merge!
...
**To do** In Merged_Data, rename() the Park column to JOMU_Species. Change the data within the column so that JOMU is TRUE and the null values are FALSE
```{r}
...
**To do** Repeat the steps starting with finding distinct() JOMU records for Genus (using only Genus rather than Genus and Species). Once merged, rename the column to JOMU_Genus and mutate to a TRUE/FALSE column.
```{r}
...
```{r}
Merged_Data %>%
  mutate(JOMU_Species = as.numeric(JOMU_Species)) %>%
  replace_na(list(JOMU_Genus = 0, JOMU_Species = 0)) %>%
  mutate(category = JOMU_Genus+JOMU_Species) %>%
  filter(category > 0) %>%
  mutate(category = (ifelse(category>1,"Species","Genus"))) %>%

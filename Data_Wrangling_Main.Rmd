...
1. Send Laura your GitHub username so that she can give you editing access to the repo.
    ...
    *Note for recording listeners*: You will not have editing access to the repository, you can still create a clone and follow along with a local copy (rather than a branch). Please skip steps 2, 3 and 6.
...
```
```{r}
# You can add more arguments to the function. The inclusion of the integer "4" will show the first 10 rows instead of the default 5. 

head(groundhog_raw, 4)
```

**To do** Following the same format as above, use the tail() function to view the last 3 rows of groundhog_raw.
...
**Did Punxsutawney Phil predict an early spring (no shadow) correctly (average temperature above freezing) in February in Pennsylvania between the years 1900 and 2000? **
...
c. Filter for predict early spring (Punxsutawney Phil = No Shadow)

d. Filter for temperature above freezing (Average Pennsylvania temp > 32)

e. Convert and filter year range (year>1900 and year < 2000)

f. Bonus: Change column layout to include only the data we want.
...
**To Do** Print out the current column names of groundhog_clean using the colnames() function.
```{r}
colnames(groundhog_raw)
```

Tidyverse uses a pipeline (%>%) to denote a continuous chain of functions (the first output becomes the next input)

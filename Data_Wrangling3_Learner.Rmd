...
```{r}
NPSutils::getDataPackages(2233056)
...
```{r}
# Create a new dataframe called CRMO_Raw. Use read_xls() to pull in the data we downloaded
...
# New Dataframe <- Old dataframe %>% functions
CRMO_Edit <- CRMO_Raw %>% 
  select(-FirstOfRank, -FirstOfSubspecies, -FirstOfAuthority_subsp) %>% drop_na()
...
CRMO_ForMerge <- CRMO_Edit %>% 
  #New name = old name
  rename(CRMO_Genus = FirstOfGenus, CRMO_Species = FirstOfspecies)
...
*To do* Create a new dataset called Merged_Data. Left Join the two ForMerge datasets on Genus and species columns
```{r}
Merged_Data <- left_join(CRMO_ForMerge, JOMU_ForMerge, by=c('CRMO_Genus'='Genus', 'CRMO_Species'='Species_edit'))
...
```{r}
nrow(CRMO_ForMerge %>%
        select(CRMO_Genus,CRMO_Species)%>%
        distinct())
```
...
```{r}
Merged_Data <- left_join(CRMO_ForMerge, JOMU_ForMerge2, by=c('CRMO_Genus'='Genus', 'CRMO_Species'='Species_edit'))
```
...
Merged_Data <- left_join(Merged_Data, JOMU_ForMerge3, by=c('CRMO_Genus'='Genus'))
